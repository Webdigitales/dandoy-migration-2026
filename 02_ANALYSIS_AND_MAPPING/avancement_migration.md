...
| Demander au prestataire/dev Magento : existe-t-il un ID de transaction pour PayPlug, PayPal Express et Klarna (~2 850 commandes, 7,3%) ? **Confirmé absent** de la liste d'attributs order Magento Admin (vérifié 4 août 2026, seul `mollie_transaction_id` y figure) — probablement stocké hors des attributs order plats (`sales_order_payment.additional_information`), à confirmer par le dev | Moyenne | Question posée (4 août 2026) — à défaut, `Note` restera vide pour ces commandes |
